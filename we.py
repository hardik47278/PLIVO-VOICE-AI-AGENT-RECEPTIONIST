...
# ── Custom energy-based VAD constants (mirrors the Node.js logic) ──
# Lowered from a typical 500 default to better catch low/quiet voices.
ENERGY_THRESHOLD = 600
FRAMES_REQUIRED = 2
INTERRUPT_COOLDOWN = 500
INTERRUPT_HOLD_MS = 600
MIN_SPEECH_STOP_GAP_MS = 150
...
def get_energy(pcm16_bytes: bytes) -> float:
    """
    Mirrors the Node.js getEnergy() function: average absolute amplitude
    of 16-bit PCM samples. Used as a custom, additional speech-activity
    signal alongside Gemini's own built-in VAD (automatic_activity_detection),
    tuned with a lower threshold to better catch quiet/low-volume voices.
    """
    if not pcm16_bytes:
        return 0.0
    ...
    for i in range(sample_count):
        sample = int.from_bytes(pcm16_bytes[i * 2:i * 2 + 2], byteorder="little", signed=True)
        total += abs(sample)
...
def build_gemini_config(is_inbound=True, campaign_data=None, lead_data=None, resumption_handle=None):
    return types.LiveConnectConfig(
        ...
        temperature=0.4,
        # ── thinking_budget=0: disable internal chain-of-thought
        # reasoning before responding. Removes 200-500ms of hidden
        # latency on every turn with no quality loss for this use case
        # (receptionist script is deterministic enough to not need it).
        ...
        realtime_input_config=types.RealtimeInputConfig(
            # Explicitly force barge-in: as soon as VAD detects the caller
            # speaking, cut the model's current turn immediately — instead
            # of relying on unset/default behavior, which was unreliable
            # specifically on the long first-turn greeting.
            activity_handling=types.ActivityHandling.START_OF_ACTIVITY_INTERRUPTS,
            automatic_activity_detection=types.AutomaticActivityDetection(
                disabled=False,
                start_of_speech_sensitivity=types.StartSensitivity.START_SENSITIVITY_HIGH,
                end_of_speech_sensitivity=types.EndSensitivity.END_SENSITIVITY_HIGH,
                # Lower padding = commits to "user started speaking" faster.
                prefix_padding_ms=150,
                # Lower silence_duration_ms = faster end-of-turn detection.
...
async def handle_media_stream(
    ...
    # ── Custom energy-VAD state (mirrors Node.js module-level state) ──
    ...
    async def plivo_receiver(session):
        nonlocal caller_phone, plivo_closed, speech_frame_count, last_interrupt_at, speech_active, speech_active_since, last_silence_at
        ...
        try:
            async for message in plivo_ws.iter_text():
                ...
                if event == "media":
                    ...
                    # ── Custom energy-based VAD check (additional signal,
                    # alongside Gemini's own automatic_activity_detection,
                    # not replacing it). Threshold lowered for low voices. ──
                    energy = get_energy(pcm16k)
                    now_ms = int(time.time() * 1000)

                    if energy >= ENERGY_THRESHOLD:
                        speech_frame_count += 1
                        last_silence_at = 0
                        if not speech_active and speech_frame_count >= FRAMES_REQUIRED:
                            speech_active = True
                            speech_active_since = now_ms
                            if DEBUG:
                                print(f"[ENERGY VAD] speech start detected energy={energy:.1f}")
                            if now_ms - last_interrupt_at >= INTERRUPT_COOLDOWN:
                                last_interrupt_at = now_ms
                                if DEBUG:
                                    print(f"[ENERGY VAD] interrupt signal (cooldown clear)")
                                # ── ACTUAL INTERRUPT ACTION ──
                                try:
                                    await plivo_ws.send_text(json.dumps({
                                        "event": "clearAudio",
                                        "stream_id": stream_id
                                    }))
                                    print(f"🛑 [{get_ist_string()}] [ENERGY VAD] Barge-in → cleared Plivo audio")
                                except Exception as clear_err:
                                    print(f"[ENERGY VAD CLEAR ERROR] type={type(clear_err).__name__} msg={clear_err}")
                                # ── Also tell Gemini itself an activity started,
                                # so it interrupts its own in-flight turn server-side
                                # (clearing Plivo alone only stops local playback;
                                # Gemini keeps generating unless told here). ──
                                try:
                                    await session.send_realtime_input(
                                        activity_start=types.ActivityStart()
                                    )
                                    if DEBUG:
                                        print(f"[ENERGY VAD] sent activity_start to Gemini")
                                except Exception as activity_err:
                                    print(f"[ENERGY VAD ACTIVITY_START ERROR] type={type(activity_err).__name__} msg={activity_err}")
                    else:
                        speech_frame_count = 0
                        if speech_active:
                            if last_silence_at == 0:
                                ...
                                if now_ms - speech_active_since >= INTERRUPT_HOLD_MS:
                                    speech_active = False
                                    if DEBUG:
                                        print(f"[ENERGY VAD] speech end detected (held {now_ms - speech_active_since}ms)")
