...
async def add_conversation_entry(request_uuid, entry, is_inbound, db, inbound_col, outbound_col, broadcast_fn=None):
    ...
    try:
        ...
        if not transcript:
            transcript = await create_transcript(request_uuid, "Unknown", is_inbound, db, inbound_col, outbound_col)
            if not transcript:
                return
...
async def generate_auto_summary(request_uuid, is_inbound, db, inbound_col, outbound_col, broadcast_fn=None, gemini_api_key=None):
    ...
    try:
        ...
        if gemini_api_key and conversation_text.strip():
            try:
                import httpx
...
{conversation_text}"""
                    ...
                    # ── FIX: guard against missing/blocked/error responses ──
                    candidates = result.get("candidates")
                    if not candidates:
                        error_info = result.get("error", {})
                        prompt_feedback = result.get("promptFeedback", {})
                        print(f"⚠️ Gemini summary: no candidates returned. "
                              f"error={error_info} promptFeedback={prompt_feedback}")
                    else:
                        content = candidates[0].get("content", {})
                        parts = content.get("parts", [])
                        if not parts:
                            finish_reason = candidates[0].get("finishReason", "unknown")
                            print(f"⚠️ Gemini summary: empty parts. finishReason={finish_reason}")
                        else:
                            ...
                            if json_match:
                                ...
                                print(f"⚠️ Gemini summary: no JSON found in response text")
            except Exception as e:
