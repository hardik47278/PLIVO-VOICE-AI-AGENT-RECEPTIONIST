...
SILERO_THRESHOLD = 0.5      # 0.0–1.0: higher = less sensitive, lower = more sensitive
...
def is_speech_silero(pcm16k: bytes) -> bool:
    """
    Silero VAD: expects 16kHz mono PCM16 audio.
    512 samples = 32ms at 16kHz = 1024 bytes — recommended window size.
    Returns a confidence score; compared against SILERO_THRESHOLD.
    """
    if _silero_model is None:
        return False
    try:
        import torch
        # 512 samples = 1024 bytes at 16kHz (Silero recommended window)
        frame = pcm16k[:1024]
        if len(frame) < 1024:
        ...
        try:
            async for message in plivo_ws.iter_text():
                ...
                if event == "media":
                    ...
                    # No RMS gate needed — Silero handles noise/silence internally
                    speech_detected = is_speech_silero(pcm16k)
