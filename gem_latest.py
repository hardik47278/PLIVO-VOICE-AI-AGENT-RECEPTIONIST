...
def build_gemini_config(is_inbound=True, campaign_data=None, lead_data=None, resumption_handle=None):
    return types.LiveConnectConfig(
        ...
        temperature=0.4,
        # Disable internal "thinking" tokens — this agent does scripted,
        # low-complexity tasks (greeting, name collection, appointment
        # booking, simple lookups), so reasoning overhead before the
        # model starts producing audio is pure latency with little
        # benefit here. thinking_budget=0 = disabled, -1 = automatic.
        # If response quality drops on harder questions, raise this
        # back up (try a small positive budget) rather than going to -1.
        ...
        realtime_input_config=types.RealtimeInputConfig(
            ...
            # of relying on unset/default behavior. THIS WAS MISSING in
            # the previous version of this file, which is why barge-in /
            # latency tuning appeared to have "no effect" — you were
            # running without it even though it was added elsewhere.
            ...
            automatic_activity_detection=types.AutomaticActivityDetection(
                ...
                # Lower silence_duration_ms = Gemini decides "user is done
                # talking" faster, especially for short replies like "Yes."
                # Trade-off: too low risks cutting off people who pause
                # mid-sentence. Raise back toward 400-500ms if real
                # sentences start getting clipped in testing.
...
async def handle_media_stream(
    ...
    async def gemini_receiver(session):
        ...
        try:
            ...
            while True:
                        ...
                        if sc.model_turn:
                            for part in sc.model_turn.parts:
                                if part.inline_data and part.inline_data.data:
                                    # Skip sending audio if Plivo is already
                                    # known to be closed — avoids spamming
                                    # "websocket.send after websocket.close"
                                    # errors for the rest of an in-flight
                                    # Gemini response after the caller hangs up.
                                    if plivo_closed:
                                        continue
                ...
                if plivo_closed:
                    # Plivo is gone — no point draining further Gemini
                    # output, nobody is listening on the other end.
                    break
    ...
    for attempt in range(MAX_SESSION_RETRIES + 1):
        ...
        try:
            async with client.aio.live.connect(model=GEMINI_MODEL1, config=config) as session:
