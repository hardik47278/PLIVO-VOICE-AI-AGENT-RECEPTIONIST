...
# ── WebRTC VAD constants ──
# mode: 0=least aggressive, 1=balanced, 2=recommended for phone, 3=most aggressive
WEBRTC_VAD_MODE = 2
FRAMES_REQUIRED = 3         # consecutive speech frames before barge-in fires
INTERRUPT_COOLDOWN = 1000    # ms between barge-in triggers
INTERRUPT_HOLD_MS = 600     # ms caller must speak before speech_active resets
MIN_SPEECH_STOP_GAP_MS = 150 # ms silence before speech considered ended
...
def is_speech_webrtc(vad, pcm16k: bytes) -> bool:
    """
    WebRTC VAD requires exactly 10/20/30ms frames at 16kHz.
    20ms at 16kHz = 640 bytes (320 samples x 2 bytes).
    We take the first 640 bytes of the incoming chunk.
    ...
    frame = pcm16k[:640]
    if len(frame) < 640:
        frame = frame + b'\x00' * (640 - len(frame))
    try:
...
async def handle_media_stream(
    ...
    last_silence_at = 0
    speech_start_ms = 0          # tracks continuous speech start time for 200ms gate

    # ── Initialize WebRTC VAD ──
    vad = None
    if webrtcvad is not None:
        vad = webrtcvad.Vad(WEBRTC_VAD_MODE)
        print(f"✅ WebRTC VAD initialized (mode={WEBRTC_VAD_MODE})")
    else:
        print("⚠️ webrtcvad not installed — barge-in VAD disabled. Run: pip install webrtcvad-wheels")
    ...
    async def plivo_receiver(session):
        nonlocal caller_phone, plivo_closed, speech_frame_count, last_interrupt_at, speech_active, speech_active_since, last_silence_at, speech_start_ms
        ...
        try:
            async for message in plivo_ws.iter_text():
                ...
                if event == "media":
                    ...
                    # ── Energy gate: skip WebRTC VAD on low-energy frames ──
                    rms = audioop.rms(pcm16k, 2)
                    if rms > 400:
                        speech_detected = is_speech_webrtc(vad, pcm16k)
                    else:
                    ...
                    if speech_detected:
                        ...
                        # Track continuous speech start time
                        ...
                        if not speech_active and speech_frame_count >= FRAMES_REQUIRED and speech_duration >= 100:
                            ...
                            if DEBUG:
                                print(f"[WEBRTC VAD] speech start detected (rms={rms}, duration={speech_duration}ms)")
                            if now_ms - last_interrupt_at >= INTERRUPT_COOLDOWN:
                                last_interrupt_at = now_ms
                                if DEBUG:
                                    print(f"[WEBRTC VAD] interrupt signal (cooldown clear)")
                                ...
                                try:
                                    ...
                                    print(f"🛑 [{get_ist_string()}] [WEBRTC VAD] Barge-in → cleared Plivo audio")
                                except Exception as clear_err:
                                    print(f"[WEBRTC VAD CLEAR ERROR] type={type(clear_err).__name__} msg={clear_err}")
                                # ── Tell Gemini to stop generating server-side ──
                                try:
                                    await session.send_realtime_input(
                                        activity_start=types.ActivityStart()
                                    )
                                    if DEBUG:
                                        print(f"[WEBRTC VAD] sent activity_start to Gemini")
                                except Exception as activity_err:
                                    print(f"[WEBRTC VAD ACTIVITY_START ERROR] type={type(activity_err).__name__} msg={activity_err}")
                    else:
                        speech_frame_count = 0
                        speech_start_ms = 0   # reset continuous speech timer on any silence
                        if speech_active:
                            if last_silence_at == 0:
                                ...
                                if now_ms - speech_active_since >= INTERRUPT_HOLD_MS:
                                    speech_active = False
                                    if DEBUG:
                                        print(f"[WEBRTC VAD] speech end detected (held {now_ms - speech_active_since}ms)")
