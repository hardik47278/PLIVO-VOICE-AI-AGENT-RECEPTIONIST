...
CALIBRATION_FRAMES = 50       # calibrate on first 50 frames only
THRESHOLD_OFFSET = 80         # lower = catches soft voice
MIN_THRESHOLD = 120           # never go below this
SPEECH_FRAMES_TRIGGER = 5     # 5 consecutive frames to trigger barge-in
...
async def handle_media_stream(
    ...
    async def plivo_receiver(session):
        ...
        # ── Adaptive VAD state (one-time calibration) ──────
        quiet_frames = deque(maxlen=CALIBRATION_FRAMES)
        noise_floor = 50.0
        dynamic_threshold = 170.0
        calibrated = False
        speech_frames = 0
        ...
        try:
            async for message in plivo_ws.iter_text():
                ...
                if event == "media":
                    ...
                    # ── Adaptive RMS Barge-in (Layer 1) ───────────
                    rms = audioop.rms(pcm16k, 2)

                    if not calibrated:
                        # one-time calibration: collect quiet frames
                        if rms < dynamic_threshold * 0.8:
                            quiet_frames.append(rms)

                        if len(quiet_frames) >= CALIBRATION_FRAMES:
                            noise_floor = statistics.median(quiet_frames)
                            dynamic_threshold = max(
                                noise_floor + THRESHOLD_OFFSET,
                                MIN_THRESHOLD
                            )
                            calibrated = True
                            print(
                                ...
                                f"threshold={dynamic_threshold:.1f}"
                            )

                    # speech detection — works before and after calibration
                    if rms > dynamic_threshold:
                        ...
                        if speech_frames >= SPEECH_FRAMES_TRIGGER:
                            try:
                                ...
                                print(
                                    f"🛑 [{get_ist_string()}] [LOCAL VAD] Barge-in "
                                    f"rms={rms} "
                                    f"noise_floor={noise_floor:.1f} "
                                    f"threshold={dynamic_threshold:.1f}"
                                )
                            except Exception as e:
                                print(f"[RMS BARGE ERROR] {e}")
                            speech_frames = 0
                    else:
                        speech_frames = 0
                    # ── End Adaptive RMS Barge-in ──────────────────
