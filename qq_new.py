...
# ── Plivo credentials (for programmatic hangup) ──
...
# ── Slow tools that warrant hold audio ──
...
# ── Load hold.mp3 once at startup → convert to mulaw base64 ──
def _load_hold_audio_from_mp3(path: str = "hold.mp3") -> str:
    try:
        ...
        print(f"⚠️ Failed to load hold.mp3: {e} — hold audio disabled")
        return ""
...
async def handle_media_stream(
    ...
    # ── Use module-level hold audio loaded from hold.mp3 ──
    ...
    async def gemini_receiver(session):
        nonlocal resumption_handle, ai_last_spoke_at, plivo_closed, is_transferring
        event_counter = 0
        pending_calls = set()
                ...
                async for response in session.receive():
                    ...
                    if response.tool_call:
                        for fc in response.tool_call.function_calls:
                            ...
                            # ── Special case: end_call — hang up after goodbye ──
                            if fn_name == "end_call":
                            ...
                            if fn_name == "transfer_to_human":
                                ...
                                print(f"[TRANSFER] AI requested transfer to human agent (call_uuid={request_uuid}, reason={reason})")
                                active_calls[request_uuid]["is_inbound"] = is_bound
                                ...
                                async def _delayed_transfer():
                                    ...
                                    transfer_call(request_uuid,f"h")
                                    plivo_closed=True
                                    try:
                                        await plivo_ws.close()
                                    except Exception as e:
                                        print(f"[TRANSFER CLOSE ERROR] {e}")
                                        pass         

                                async def _delayed_hangup():
                                    nonlocal plivo_closed
                                    await asyncio.sleep(2.5)  # let goodbye audio finish playing
                                    hangup_call(request_uuid)
                                    plivo_closed = True
                                    try:
                                        await plivo_ws.close()
                                    except Exception:
                            ...
                            if fn_name in SLOW_TOOLS:
                                asyncio.create_task(play_hold_audio())
                                print(f"[HOLD] Triggered hold audio for slow tool: {fn_name}")
